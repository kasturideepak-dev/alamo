...
async def main(node, prefix, shots):
    ...
    async with async_playwright() as p:
        ...
        # dismiss the cookie bar if present
        try:
            await pg.click("text=Do not allow cookies", timeout=3000); await pg.wait_for_timeout(1200)
        except Exception: pass
        for i in range(shots):
            await pg.screenshot(path=f"scratch/{prefix}-{i}.png")
            await pg.mouse.wheel(0, 1900)
